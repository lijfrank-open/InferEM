...
class Translator(object):
    ...
    def beam_search(self, batch, max_dec_step):
        ...
        with torch.no_grad():
            ...
            context_input = batch["context_batch"] 
            concept_input = batch["concept_batch"] 

            semantic_embed = self.model.embedding(context_input) 
            concept_semantic_embed = self.model.embedding(concept_input) 
            ...
            context_resp = self.model.encoder(concept_context,
                                        concept_context_mask)

            ROOT_resp = context_resp[0, :, :] 
            emotion_logit = self.model.identify(ROOT_resp) 
            ...
            n_bm = self.beam_size 
            len_s, n_inst, d_h = context_resp.size()  
