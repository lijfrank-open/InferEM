...
class EncoderLayer_cr(nn.Module):
    """
    Represents one Encoder layer of the Transformer Encoder
    Refer Fig. 1 in https://arxiv.org/pdf/1706.03762.pdf
    NOTE: The layer normalization step has been moved to the input as per latest version of T2T
    ...
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        super(EncoderLayer_cr, self).__init__()
        ...
        self.dropout = nn.Dropout(layer_dropout)
        self.layer_norm_mha = LayerNorm(hidden_size)
        self.layer_norm_mha1 = LayerNorm(hidden_size)
        self.layer_norm_ffn = LayerNorm(hidden_size)

    def forward(self, inputs, inputs1, mask=None):
        x = inputs
        x1 = inputs1

        x_norm = self.layer_norm_mha(x)
        x1_norm = self.layer_norm_mha1(x1)

        y, _ = self.multi_head_attention(x_norm, x1_norm, x1_norm, mask)
